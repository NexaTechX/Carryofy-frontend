        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto">
          {testimonials.map((testimonial, index) => (
            <motion.div
              ...
              className="bg-[#FAFAFA] rounded-2xl p-6 sm:p-8 border border-gray-100 shadow-sm hover:shadow-lg transition-shadow flex flex-col h-full"
            >
              <Quote className="w-10 h-10 text-[#FF6B00]/20 mb-4" aria-hidden />
              <div className="flex items-center gap-2 mb-4">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-4 h-4 sm:w-5 sm:h-5 ${
                      i < testimonial.rating ? 'fill-[#FF6B00] text-[#FF6B00]' : 'text-gray-200'
                    }`}
                  />
                ))}
              </div>
              <blockquote className="text-[#111111] text-sm sm:text-base leading-relaxed mb-6 grow">
              ...
              <div className="flex items-center gap-4 pt-4 border-t border-gray-200">
                ...
                <div>
                  <h3 className="font-semibold text-[#111111]">{testimonial.name}</h3>
                  <p className="text-sm text-gray-500">
                    {testimonial.location} · {testimonial.tag}
                  </p>
                </div>
              </div>
            </motion.div>
